...
public class RBTree {
	...
	/**
	 * FIXME: this does not work properly yet for augmented red-black trees since it
	 * doesn't update nodes. Need to figure out exactly from which points we need to
	...
	protected Comparable getNodeValue(RBNode node) {
		return node.getData();
	}
	...
	private void treeInsert(RBNode z) {
		...
		while (x != null) {
			y = x;
			if (getNodeValue(z).compareTo(getNodeValue(x)) < 0) {
				x = x.getLeft();
			} else {
				x = x.getRight();
			}
		...
		if (y == null) {
			...
			if (getNodeValue(z).compareTo(getNodeValue(y)) < 0) {
				y.setLeft(z);
			} else {
				y.setRight(z);
			}
	...
	/**
	 * Restores red-black property to tree after splicing out node during deletion.
	 * Note that x may be null, which is why xParent must be specified.
	 */
	...
	private int verifyFromNode(RBNode node) {
		...
		if (i != j) {
			...
			throw new RuntimeException(
					"Verify failed (4) (left black count = " + i + ", right black count = " + j + ")");
		}
