...
public class RodCutting {
     ...
     * 
     * @param prices
     *            array of the price, index is the length, value is the price.
     *            expecting the value of index 0 to be 0. always like {0, non-zero
     *            ...}
     * @param len
     *            the length of the raw rod
     * @return max revenue
     */
    ...
    public int cutRodMaxRevenueTopBottomMemAux(int[] prices, int len, int[] revenue) {
        ...
        for (int j = 1; len >= j && j < prices.length; j++) {
            revenue[len] = Math.max(revenue[len],
                    prices[j] + cutRodMaxRevenueTopBottomMemAux(prices, len - j, revenue));
        }
        return revenue[len];
    }
